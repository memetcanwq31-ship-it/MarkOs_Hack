#!/usr/bin/env python3
"""
Instagram ID & Konum İstihbarat Aracı - v4.1 (REAL + RATE LIMIT)
☑ Instagram'dan GERÇEK veri çeker (instaloader motoru)
☑ ID'den Username bulma (gerçek Profile.from_id)
☑ Rastgele bekleme süresi (10-25 saniye) - Instagram engelini aşmak için
...
# ─── BEKLEME MOTORU (INSTAGRAM ENGELİNİ AŞMAK İÇİN) ──────────────────────────
def instagram_bekle():
    """Her sorgu arası 10-25 saniye rastgele bekleme"""
    bekleme_suresi = random.randint(10, 25)
    print(f"{Fore.YELLOW}[⏳] Instagram engeline takılmamak için {bekleme_suresi} saniye bekleniyor...")
...
def kaydet(data: dict, prefix: str = "rapor"):
    try:
        ...
        with open(txt, "w", encoding="utf-8") as f:
            f.write("INSTAGRAM KONUM İSTİHBARAT RAPORU v4.1\n")
            f.write("="*50 + "\n")
...
def coklu_sorgu():
    ...
    for i, u in enumerate(liste, 1):
        ...
        # ← BEKLEME EKLENDİ (her sorgu sonrası)
        instagram_bekle()
...
def menu():
    while True:
        print(f"\n{Fore.RED}{'█'*60}")
        print(f"{Fore.RED}█{Fore.CYAN}  INSTAGRAM ID & KONUM İSTİHBARAT ARACI v4.1{' '*11}{Fore.RED}█")
        print(f"{Fore.RED}█{Fore.CYAN}  GERÇEK VERİ | instaloader | 10-25sn rastgele bekleme{' '*2}{Fore.RED}█")
        print(f"{Fore.RED}{'█'*60}")
