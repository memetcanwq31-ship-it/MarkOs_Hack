...
# Clear the file and write part 1
with open('/mnt/agents/output/PiwXware_v4_Full.py', 'w', encoding='utf-8') as f:
    f.write('''#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
╔══════════════════════════════════════════════════════════════════╗
║           PIWXWARE PRO - ETİK SİBER GÜVENLİK SUİTİ v4.0        ║
║    Yalnızca yetkili sistemlerde ve eğitim amaçlı kullanım içindir║
╚══════════════════════════════════════════════════════════════════╝
"""
...
def banner():
    ...
    ║  {Fore.RED}╚═╝     ╚═╝ ╚══╝╚══╝ ╚═╝  ╚═╝ ╚══╝╚══╝ ╚═╝  ╚═╝╚═╝  ╚═╝{Fore.CYAN}    ║
    ║                                                               ║
    ║  {Fore.YELLOW}ETIK SIBER GUVENLIK & PENETRASYON TESTI SUITI v4.0 PRO{Fore.CYAN}   ║
...
with open('/mnt/agents/output/PiwXware_v4_Full.py', 'a', encoding='utf-8') as f:
    f.write(part2)

print("Part 2 OK (Modules 1-5)")
...
print("Part 3 OK (Modules 6-10)")


# PART 4 - Modules 11-15 (fixed escaping)
part4 = """
# ═══════════════════════════════════════════════════════════════
...
def port_listener():
    ...
    try:
        ...
        while True:
            ...
            client.send(b"PiwXware Listener v4.0 - Baglanti aktif\n")
...
print("Part 5 OK (Modules 16-20)")

Module 21 OK


# Write remaining modules efficiently using a list
lines = []

# Module 22
lines.append("# ═══════════════════════════════════════════════════════════════")
lines.append("# MODUL 22: CMS TEKNOLOJI TESPITI")
lines.append("# ═══════════════════════════════════════════════════════════════")
lines.append("")
lines.append("def cms_detect():")
lines.append("    banner()")
lines.append('    log("CMS TEKNOLOJI TESPITI", "info")')
lines.append('    hedef = input(f"{Fore.GREEN}[?] Hedef URL: {Fore.WHITE}").strip()')
lines.append('    if not hedef:')
lines.append('        log("URL bos!", "error")')
lines.append('        return')
lines.append('    if not hedef.startswith(("http://", "https://")):')
lines.append('        hedef = "https://" + hedef')
lines.append('    try:')
lines.append('        response = requests.get(hedef, timeout=10, headers={"User-Agent": "PiwXware/4.0"})')
lines.append('        html = response.text.lower()')
lines.append('        headers_str = str(response.headers).lower()')
lines.append('        cms_imzalari = {"WordPress": ["/wp-content/", "wp-json"], "Joomla": ["joomla"], "Drupal": ["drupal"], "React": ["react"], "Vue.js": ["vue"], "Angular": ["angular"], "Laravel": ["laravel"], "Django": ["django"], "Flask": ["flask"]}')
lines.append('        bulunan = []')
lines.append('        for cms, imzalar in cms_imzalari.items():')
lines.append('            for imza in imzalar:')
lines.append('                if imza.lower() in html or imza.lower() in headers_str:')
lines.append('                    if cms not in bulunan:')
lines.append('                        bulunan.append(cms)')
lines.append('        print(f"\\n{Fore.CYAN}{\'=\'*60}")')
lines.append('        if bulunan:')
lines.append('            log("Tespit edilen teknolojiler:", "success")')
lines.append('            for cms in bulunan:')
lines.append('                print(f"  {Fore.GREEN}[✓] {cms}")')
lines.append('        else:')
lines.append('            log("CMS tespit edilemedi.", "warning")')
lines.append('        server = response.headers.get("Server", "Bilinmiyor")')
lines.append('        print(f"\\n{Fore.YELLOW}[*] Sunucu: {server}")')
lines.append('        print(f"{Fore.CYAN}{\'=\'*60}")')
lines.append('    except Exception as e:')
lines.append('        log(f"CMS tespit hatasi: {e}", "error")')
lines.append('')

# Module 23
lines.append("# ═══════════════════════════════════════════════════════════════")
lines.append("# MODUL 23: WORDLIST OLUSTURUCU")
lines.append("# ═══════════════════════════════════════════════════════════════")
lines.append("")
lines.append("def wordlist_generator():")
lines.append("    banner()")
lines.append('    log("OZEL WORDLIST OLUSTURUCU", "info")')
lines.append('    kelimeler = input(f"{Fore.GREEN}[?] Anahtar kelimeler (virgulle ayirin): {Fore.WHITE}").strip().split(",")')
lines.append('    kelimeler = [k.strip() for k in kelimeler if k.strip()]')
lines.append('    if not kelimeler:')
lines.append('        log("Kelime girilmedi!", "error")')
lines.append('        return')
lines.append('    max_uzunluk = int(input(f"{Fore.GREEN}[?] Maksimum kombinasyon uzunlugu [3]: {Fore.WHITE}") or "3")')
lines.append('    sonuclar = []')
lines.append('    for r in range(1, max_uzunluk + 1):')
lines.append('        for combo in product(kelimeler, repeat=r):')
lines.append('            sonuclar.append("".join(combo))')
lines.append('    dosya_adi = f"wordlist_{datetime.now().strftime(\'%Y%m%d_%H%M%S\')}.txt"')
lines.append('    with open(dosya_adi, "w", encoding="utf-8") as f:')
lines.append('        for sifre in sonuclar:')
lines.append('            f.write(sifre + "\\n")')
lines.append('    log(f"{len(sonuclar)} kombinasyon olusturuldu: {dosya_adi}", "success")')
lines.append('')

# Module 24
lines.append("# ═══════════════════════════════════════════════════════════════")
lines.append("# MODUL 24: BASIT PAKET DINLEYICI (KENDI AG)")
lines.append("# ═══════════════════════════════════════════════════════════════")
lines.append("")
lines.append("def packet_sniffer():")
lines.append("    banner()")
lines.append('    log("BASIT PAKET DINLEYICI", "info")')
lines.append('    log("UYARI: Sadece kendi aginizda kullanin!", "warning")')
lines.append('    try:')
lines.append('        import scapy.all as scapy')
lines.append('    except ImportError:')
lines.append('        log("scapy kuruluyor...", "warning")')
lines.append('        os.system("pip install scapy")')
lines.append('        try:')
lines.append('            import scapy.all as scapy')
lines.append('        except:')
lines.append('            log("scapy kurulamadi.", "error")')
lines.append('            return')
lines.append('    paket_sayisi = int(input(f"{Fore.GREEN}[?] Kac paket yakalansin [10]: {Fore.WHITE}") or "10")')
lines.append('    log(f"{paket_sayisi} paket dinleniyor... (Ctrl+C ile durdurun)", "info")')
lines.append('    try:')
lines.append('        paketler = scapy.sniff(count=paket_sayisi, timeout=30)')
lines.append('        print(f"\\n{Fore.CYAN}{\'=\'*60}")')
lines.append('        for i, pkt in enumerate(paketler, 1):')
lines.append('            src = pkt[scapy.IP].src if scapy.IP in pkt else "N/A"')
lines.append('            dst = pkt[scapy.IP].dst if scapy.IP in pkt else "N/A"')
lines.append('            proto = pkt[scapy.IP].proto if scapy.IP in pkt else "N/A"')
lines.append('            print(f"{Fore.GREEN}[{i}] {src:<20} -> {dst:<20} | Protokol: {proto}")')
lines.append('        print(f"{Fore.CYAN}{\'=\'*60}")')
lines.append('    except PermissionError:')
lines.append('        log("Root yetkisi gerekli!", "error")')
lines.append('    except KeyboardInterrupt:')
lines.append('        log("Dinleyici durduruldu.", "warning")')
lines.append('    except Exception as e:')
lines.append('        log(f"Paket dinleyici hatasi: {e}", "error")')
lines.append('')
...
def main():
    while True:
        banner()
        print(f"""
{Fore.YELLOW}    [ 🛡️ ETIK SIBER GUVENLIK ARACLARI - v4.0 PRO ]
...
{Fore.CYAN}    [7]  {Fore.WHITE}Kali Linux Arac Entegrasyonu
