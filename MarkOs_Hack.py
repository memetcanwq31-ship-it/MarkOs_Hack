...
# -*- coding: utf-8 -*-
"""
MARKOS HACK - Pentest Toolkit v1.0
Kapsam: port tarama, web zafiyet tarama, wifi (WPA/WPS), sifre saldirisi,
Instagram halka acik profil bilgisi.
...
"""
import argparse, itertools, json, os, socket, ssl, subprocess, sys, threading, time
...
def mod_web(args):
    ...
    if args.nuclei:
        print("\n=== Nuclei Taramasi ===")
        try:
            subprocess.run(["nuclei", "-u", url, "-severity", "medium,high,critical", "-silent"], check=False)
...
def _run(cmd, check=True):
    ...
    try:
        ...
        print(f"[-] Araç bulunamadi: {cmd[0]} (sudo apt install aircrack-ng hashcat hcxtools)")
        sys.exit(1)
